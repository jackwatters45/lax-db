...
import { TeamContract } from '@lax-db/core/team/team.contract';
...
export const TeamHandlers = TeamRpcs.toLayer(
  Effect.gen(function* () {
    ...
    return {
      ...
      TeamInvitePlayer: (payload) => service.invitePlayer(payload),
      TeamRemoveMember: (payload) => service.removeTeamMember(payload),
    };
  })
